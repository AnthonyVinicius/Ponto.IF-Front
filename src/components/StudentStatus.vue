<template>
  <div class="relative flex justify-center">
    <button @click="isOpen = !isOpen"
      class="flex items-center justify-center gap-2 w-36 px-4 py-2 rounded-full font-semibold text-sm focus:outline-none transition-colors duration-200"
      :class="{
    ...
    <div v-if="isOpen"
      class="absolute mt-2 w-36 bg-white border border-gray-200 rounded-xl shadow-md z-10 overflow-hidden">
      <ul class="divide-y divide-gray-100">
        <li v-for="option in options" :key="option.value" @click="selectStatus(option.value)"
